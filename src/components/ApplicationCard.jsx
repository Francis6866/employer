...
const ApplicationCard = ({ application, isCandidate = false }) => {
    ...
    const { 
        fxn: fetchHiringStatus,
        data: dataJobs,
        loading: loadingHiringStatus 
      } = useFetch(update_applications, { job_id: application.job_id })

      const handleStatusChange = (status) => {
        fetchHiringStatus(status)
      }
    ...
    <Card>
        ...
        <CardHeader>
            <CardTitle className="flex justify-between font-bold">
                { isCandidate ? 
                `${application?.job?.title} at ${application?.job?.company?.name_of_company}` 
            : application?.name }
            <Download
                size={18}
